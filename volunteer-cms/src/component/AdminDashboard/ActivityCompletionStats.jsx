...
const ActivityCompletionStats = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // ปรับแต่งชื่อให้สั้นลงโดยตัดคำที่ไม่จำเป็น
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border rounded-lg shadow-sm">
          ...
          <p className="text-sm text-blue-600">สนใจเข้าร่วม: {interested} คน</p>
          <p className="text-sm text-green-600">เข้าร่วมสำเร็จ: {completed} คน</p>
          <p className="text-sm text-gray-500 mt-1">อัตราความสำเร็จ: {completionRate}%</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow h-full">
      ...
      <div className="h-96"> {/* เพิ่มความสูงของกราฟ */}
